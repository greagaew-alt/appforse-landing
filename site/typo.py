...
def typo(t):
    # всегда пересчитываем заново от обычных пробелов — иначе старые цепочки
    # nbsp от прошлых запусков (в том числе уже сломанные) не пересобрать
    t = t.replace(NB, u' ')
    ...
    # но только если предыдущее ещё «свободно». Если оно уже приклеено предлогом
    # на шаге выше, цепочку не наращиваем: три-четыре слова подряд без единого
    # места для переноса не помещаются в узкие колонки (ловилось на мобильном —
    # «...push и персональные предложения» слипалось в одно целое).
    parts = t.split(u' ')
    if len(parts) > 1 and parts[-1] and NB not in parts[-2]:
        parts[-2:] = [parts[-2] + NB + parts[-1]]
        t = u' '.join(parts)
